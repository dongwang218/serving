...
// regression network trained and exported by inception_export.py.
...
#include "tensorflow_serving/inception_example/inception_inference.grpc.pb.h"
#include "tensorflow_serving/inception_example/inception_inference.pb.h"
...
class InceptionServiceImpl final : public InceptionService::Service {
 public:
  explicit InceptionServiceImpl(std::unique_ptr<SessionBundle> bundle)
      : bundle_(std::move(bundle)) {
    signature_status_ = tensorflow::serving::GetClassificationSignature(
        bundle_->meta_graph_def, &signature_);
  }

  Status Classify(ServerContext* context, const InceptionRequest* request,
                  InceptionResponse* response) override {
    ...
    //Tensor input(tensorflow::DT_STRING, TensorShape({1}));
    //input.flat<tensorflow::string>()(0) = request->image_data();
    Tensor input(tensorflow::DT_STRING, TensorShape({}));
    input.scalar<string>()() = request->image_data();
...
void RunServer(int port, std::unique_ptr<SessionBundle> bundle) {
  ...
  InceptionServiceImpl service(std::move(bundle));
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    LOG(FATAL) << "Usage: inception_inference --port=9000 /path/to/export";
  }
